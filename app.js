...
//*import { eventFromStateAndMessageText } from '/fsm.js';
...
async function respondTo(message) {
    ...
    fsm.ongot_start = () => {
        lastMessage = client.sendMessage(client.chat.id, "Let's start")
    }
    ...
    while (!fsm.isFinished()) {
        ...
        if (!event || fsm.cannot(event)) {
            client.sendMessage(client.chat.id, 'I don\'t know what you mean, try /start')
            break
        }
        fsm[event](lastReply)
